...
- [ ] T104 [P] [US3] Contract test for `GET /audio/{surahId}` matching `AudioRecitation` schema in `backend/tests/Quraan.ContractTests/AudioContractTests.cs`
- [ ] T105 [P] [US3] Integration test `backend/tests/Quraan.IntegrationTests/Audio/AudioServiceTests.cs` with WireMock-stubbed Al Quran Cloud + quran.com responses covering: timings present, timings absent (`hasTimings=false, ayahTimings=[]` per FR-013), upstream 503 → API returns 502 ProblemDetails (FR-014)
- [ ] T106 [P] [US3] Unit test `backend/tests/Quraan.UnitTests/Audio/AudioServiceTests.cs` covering URL composition for Alafasy (`https://cdn.islamic.network/quran/audio-surah/128/ar.alafasy/{surahId}.mp3`) and quran.com timing-fetch with caching (R-04)
- [ ] T107 [P] [US3] Frontend unit test `frontend/src/app/features/audio/audio-player.service.spec.ts` mocking `howler` covering play/pause/resume + currentAyah$ derivation from a 250 ms interval
- [ ] T108 [P] [US3] Playwright e2e `frontend/tests/e2e/us3-audio.spec.ts` covering Play / Pause / Resume / network-fail + Retry on Surah 78; throttle the page context to the `Slow 3G` profile (spec.md §Network performance baseline); **timed assertion**: capture `performance.now()` at Play click and again on the first `Howl` `play` event, asserting the delta ≤ 2,000 ms (SC-003).

### Implementation for User Story 3

- [ ] T109 [P] [US3] Create DTOs `backend/src/Quraan.Application/Audio/AudioRecitationDto.cs`, `AyahTimingDto.cs`, `ReciterInfoDto.cs` matching `AudioRecitation` schema
- [ ] T110 [P] [US3] Create `backend/src/Quraan.Domain/Repositories/IAudioTimingProvider.cs` interface
- [ ] T111 [P] [US3] Implement `backend/src/Quraan.Infrastructure/ExternalAudio/AlQuranCloudClient.cs` (typed HttpClient via `IHttpClientFactory`, polly retry policy, 5 s timeout) composing the audio URL per R-04
- [ ] T112 [P] [US3] Implement `backend/src/Quraan.Infrastructure/ExternalAudio/QuranComClient.cs` fetching `GET /api/v4/recitations/{recitationId}/by_chapter/{chapter}`, mapping `verse_timings` → `AyahTimingDto[]`, returning empty + `hasTimings=false` on 404 / parse-error per FR-013
- [ ] T113 [US3] Implement `backend/src/Quraan.Application/Audio/AudioService.cs` composing `AudioRecitationDto` from both clients; cache the timing array in `IDistributedCache` keyed `quraan:v1:audio:{reciter}:{surah}` for 24 h (R-04, R-07); translate upstream 5xx into a typed `UpstreamAudioFailureException` carrying `502` for the controller layer
- [ ] T114 [US3] Implement `backend/src/Quraan.Api/Controllers/V1/AudioController.cs` exposing `GET /api/v1/audio/{surahId}?reciter=ar.alafasy` returning `200` / `404` / `502` per OpenAPI
- [ ] T115 [P] [US3] Implement `frontend/src/app/features/audio/audio-player.service.ts` wrapping `howler.Howl`; expose `play(surahId)`, `pause()`, `resume()`, `seek(ayahNumber)`, `currentAyah$: Observable<number|null>` driven by 250 ms `interval` while playing per R-09
- [ ] T116 [P] [US3] Implement `frontend/src/app/features/audio/audio-player.component.ts` (Play / Pause / Resume button, error banner with Retry, scrub bar)
- [ ] T117 [US3] Wire `AudioPlayerComponent` into `frontend/src/app/features/quran/surah-reader.page.ts` (mount above the Ayah list); apply `.is-playing` class to `AyahComponent` whose `numberInSurah` matches `currentAyah$` (no auto-scroll per R-09)
- [ ] T118 [US3] Add US3 keys to `frontend/src/assets/i18n/{ar,en}.json` (`audio.play`, `audio.pause`, `audio.resume`, `audio.error.unreachable`, `audio.retry`)
